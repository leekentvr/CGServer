		...
		static void Main(string[] args)
		{
			...
			using (StreamWriter writer = new StreamWriter(fileName))
			{
				using (UnixExitSignal sig = new UnixExitSignal())
				{
					...
					// 強制終了対応
					sig.SetSignal((obj, e) => {
						SignalEventArgs sig_args = e as SignalEventArgs;
						MRS_LOG_DEBUG("サーバーの停止: {0}", sig_args.SignalEnums);
						s_bIsRun = false;
						Thread.Sleep(10);
					}, (Int32)Signal.SIGINT);
					sig.SetSignal((obj, e) => {
						...
						s_bIsRun = false;
						Thread.Sleep(10);
					}, (Int32)Signal.SIGTERM);
					sig.SetSignal((obj, e) => {
						SignalEventArgs sig_args = e as SignalEventArgs;
						MRS_LOG_DEBUG("サーバーの停止: {0}", sig_args.SignalEnums);
						s_bIsRun = false;
						Thread.Sleep(10);
					}, (Int32)Signal.SIGQUIT);
					sig.SetSignal((obj, e) => {
						SignalEventArgs sig_args = e as SignalEventArgs;
						MRS_LOG_DEBUG("サーバーの停止: {0}", sig_args.SignalEnums);
						s_bIsRun = false;
