...
# include <Winsock2.h>
# include <windows.h>
// wincrypt.hで定義しているマクロと同じ名称のマクロがboringsslで定義されており、ビルドエラーになるので、対応
# undef X509_NAME
...
enum MrsConnectionType {
    ...
    MRS_CONNECTION_TYPE_TCP_SSL = 5, // @member TCP_SSL(通信経路が暗号化されたTCPを使う場合はこの値を選択します)
    MRS_CONNECTION_TYPE_MRU     = 6, // @member Monobit独自R-UDP
};
...
enum MrsError {
    ...
    MRS_ETIMEDOUT       = 60, // @member 操作はタイムアウトした
...
enum MrsConnectionError {
    ...
    MRS_WRITE_ERROR_MRU_OVER_MTU        = 14, // @member MTU以上に長いデータを送ろうとした(MRUのみ)
    MRS_WRITE_ERROR_MRU_SENDQ_FULL      = 15, // @member 送信キューがあふれている(MRUのみ)
};
...
/**
 * @cbfn MrsReadCallback
 * @brief バイナリデータを受信したときに呼び出されるコールバック関数
 * @param connection レコードを受信した接続
 * @param connection_data ...
 * @param data 送信されたデータ
 * @param data_len 送信されたデータの長さ(バイト数)
 */
typedef void (MRS_API_CC *MrsReadCallback)( MrsConnection connection, void* connection_data, const void* data, uint32 data_len );
/**
