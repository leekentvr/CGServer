...
#define MRS_VERSION     0x02000000

#include "./mrs/common.hpp"
#include "./mrs/buffer.hpp"
#include "./mrs/utility.hpp"
#include "./mrs/date_time.hpp"
#include "./mrs/opt_arg.hpp"
...
MRS_API void MRS_API_CC mrs_set_read_record_callback( MrsConnection connection, MrsReadRecordCallback callback );
    /**
     * @fn mrs_set_read_callback
     * @brief 接続において接続相手が送信したバイナリデータを受信するごとに1回呼ばれるコールバック関数を設定します
     * @param connection ...
     * @param callback ... 
     */    
MRS_API void MRS_API_CC mrs_set_read_callback( MrsConnection connection, MrsReadCallback callback );
    /**
...
MRS_API MrsConnectionType MRS_API_CC mrs_connection_get_type( MrsConnection connection );
    /**
     * @fn mrs_connection_set_path
     * @brief MrsConnectionの接続先のパスを設定します
     * @param connection 設定対象の接続
     * @param value      接続先のパス
     * @return 設定できたなら、trueを返します
     */    
MRS_API bool MRS_API_CC mrs_connection_set_path( MrsConnection connection, const char* value );
    /**
     * @fn mrs_connection_get_path
     * @brief MrsConnectionの接続先のパスを取得します
     * @param connection 設定対象の接続
     * @return MrsConnectionに設定されている接続先のパスを返します
     */    
MRS_API const char* MRS_API_CC mrs_connection_get_path( MrsConnection connection );
    /**
     * @fn mrs_write_record
...
MRS_API bool MRS_API_CC mrs_write_record( MrsConnection connection, uint16 options, uint16 payload_type, const void* payload, uint32 payload_len );
    /**
     * @fn mrs_write
     * @brief 接続に対して、バイナリデータを送信します
     * @param connection レコードを送信する対象となる接続
     * @param data 送信したいデータの先頭アドレスです。データの内容はバイナリデータで、0を含んでいても問題なく送信できます。
     * @param data_len 送信したいデータの長さ(バイト)です。
     * この関数を呼び出した時点では実際に送信されず、次の mrs_update 関数の呼び出し時に可能な限り送信しようとします。
     * ただし、OSやソケットの状態によっては、次の呼び出しで確実に送信されるとは限りません。
     *  MRSはレコードを送信するときに内部的に通し番号を付与します。 
     */    
MRS_API bool MRS_API_CC mrs_write( MrsConnection connection, const void* data, uint32 data_len );
    /**
     * @fn mrs_close
     * @brief 接続を閉じます
...
MRS_API uint32 MRS_API_CC mrs_udp_get_mtu();
    /**
     * @fn mrs_mru_set_client_peer_limit
     * @brief MRUプロトコルのクライアント接続の上限値の設定
     * @param value 上限値
     */
MRS_API void MRS_API_CC mrs_mru_set_client_peer_limit( uint32 value );
    /**
     * @fn mrs_mru_get_client_peer_limit
     * @brief MRUプロトコルのクライアント接続の上限値の取得
     * @return 上限値
     */
MRS_API uint32 MRS_API_CC mrs_mru_get_client_peer_limit();
    /**
     * @fn mrs_mru_set_sendq_size
     * @brief MRUプロトコルの送信キューサイズの設定
     * @param value 送信キューサイズ
     */
MRS_API void MRS_API_CC mrs_mru_set_sendq_size( uint32 value );
    /**
     * @fn mrs_mru_get_sendq_size
     * @brief MRUプロトコルの送信キューサイズの取得
     * @return 送信キューサイズ
     */
MRS_API uint32 MRS_API_CC mrs_mru_get_sendq_size();
}
